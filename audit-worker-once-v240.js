...
if (!store.configured) {
  throw new Error('SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required for the Audit worker.');
}
...
function publicPageFromIntake(intake) {
  return intake?.clinic?.websiteUrl ||
    intake?.clinic?.website ||
    intake?.fields?.websiteUrl ||
    intake?.fields?.website ||
    null;
}
...
async function runEvidencePipeline({ jobId, sessionId, intake, delivery }) {
  ...
  if (!pageUrl) {
    throw new Error('Confirmed Audit intake has no public clinic page URL.');
  }

  const root =
    process.env.EDEN_AUDIT_OUTPUT_ROOT ||
    path.resolve(process.cwd(), 'audit-output', 'jobs');
  ...
  fs.writeFileSync(
    path.join(outputDir, 'durable-job.json'),
    JSON.stringify({ jobId, sessionId, delivery }, null, 2)
  );
  ...
  const storage = await evidenceStorage.uploadDirectory(
    jobId,
    manifest.outputDir
  );
...
main().catch(error => {
  console.error('Eden Audit one-off fatal error:', error);
  process.exitCode = 1;
});
