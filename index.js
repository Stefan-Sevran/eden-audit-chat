...
async function launchAuditOneOffV240(jobId) {
  ...
  if (!response.ok) {
    const error = new Error(
      `Render one-off launch failed (${response.status}).`
    );
    error.renderStatus = response.status;
    error.renderData = data;
    throw error;
  }

  console.log(
    "Render one-off Audit launched:",
    data?.id || data
  );
  ...
  try {
    const sheetsResult =
      await postJsonToGoogleSheets(
        ...
        leadId: bookingRecordId,
        sessionId: `${sessionId}::${bookingRecordId}`,
        clinicId,
