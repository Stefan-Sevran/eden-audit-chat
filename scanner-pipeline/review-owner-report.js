...
.readiness-badge{display:inline-flex;margin:8px 0 2px;border:1px solid var(--line);background:var(--soft);border-radius:999px;padding:5px 8px;font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:.07em}.action-evidence{margin-top:10px;padding:11px 12px;border-radius:12px;background:#f6f8f7;border:1px solid var(--line)}.action-evidence-kicker{font-size:9px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:var(--muted);margin-bottom:4px}.action-evidence-snippet{font-size:11px;color:var(--muted);margin-top:4px;line-height:1.45}.action-evidence-meta{font-size:9px;font-weight:800;color:var(--ink);margin-top:6px}.action-edit{border:1px solid var(--line);border-radius:14px;padding:14px;margin:10px 0;background:#fbfcfb}.action-edit.off{opacity:.58}.action-edit-top{display:flex;gap:10px;align-items:center}.action-edit-top input[type="checkbox"]{width:auto}.action-edit-badges{display:flex;gap:6px;flex-wrap:wrap;margin:8px 0}.action-edit-badges span{font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:.08em;border-radius:999px;padding:4px 7px;background:var(--soft)}.action-edit textarea{min-height:68px}.action-source{font-size:10px;color:var(--muted);word-break:break-word;margin-top:8px}.resolution-box{margin-top:12px;padding:12px;border-radius:10px;background:#f7f9f8;border:1px dashed #cad7d1}.resolution-box.blocking{background:#fff8f6;border-color:#e0b5ac}.resolution-box select,.resolution-box textarea{margin-top:6px}.resolution-box textarea{min-height:68px}.resolution-status{font-size:11px;font-weight:800;margin-top:7px}.resolution-status.ok{color:#17603b}.resolution-status.pending{color:#8c3e2f}
...
function renderActionPlanEvidencePreview(card,item,index){
  const presentation=state?.actionPlanPresentation?.items?.find(x=>x.id===item.id);
  if(!presentation)return;
  if(presentation.readiness){
    const badge=make('div','readiness-badge',presentation.readiness.label);
    badge.title=presentation.readiness.reason||'';
    card.appendChild(badge);
  }
  if(presentation.evidencePreview){
    const ev=make('div','action-evidence');
    ev.appendChild(make('div','action-evidence-kicker','Why this is prioritized'));
    ev.appendChild(make('strong','',presentation.evidencePreview.title||'Supporting evidence'));
    if(presentation.evidencePreview.snippet){
      ev.appendChild(make('div','action-evidence-snippet',presentation.evidencePreview.snippet));
    }
    const meta=make('div','action-evidence-meta','Confidence: '+(presentation.evidencePreview.tier||item.confidence||'unknown'));
    ev.appendChild(meta);
    card.appendChild(ev);
  }
}
function renderActionPlan(){
  ...
  items.forEach(function(item,index){
    ...
    renderActionPlanEvidencePreview(card,item,index);
