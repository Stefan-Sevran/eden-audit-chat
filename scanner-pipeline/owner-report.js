...
const {buildClinicActionPlan}=require('./action-plan-v250');
const {buildActionPlanPresentation}=require('./action-plan-presentation-v251');
...
function buildOwnerReviewTemplate(model){
  return {
    ...
    actionPlan:{items:(model.actionPlan?.items||[]).map(x=>({id:x.id,include:true,title:x.title,action:x.action,owner:x.owner,timing:x.timing}))},
...
function buildOwnerReportModel(manifest,{demoUrl=null,proofUrl=null,ownerReview=null,ownerReviewPath=null}={}){
  ...
  const actionPlan=buildClinicActionPlan(manifest,review&&review.status!=='invalid'?review:null);
  const actionPlanPresentation=buildActionPlanPresentation(manifest,actionPlan);
  const model={
    schemaVersion:VERSION,
...
function renderOwnerReport(model){
  ...
  const quickWins=model.actionPlanPresentation?.quickWins||[];
  const quickWinsHtml=quickWins.length?`<section class="section quick-wins"><div class="eyebrow">Fastest momentum</div><h2>Quick wins this week</h2><div class="quick-win-grid">${quickWins.map((item,i)=>`<article class="quick-win"><div class="quick-win-kicker">Quick win ${i+1}</div><h3>${esc(item.title)}</h3><p>${esc(item.action)}</p><div class="quick-win-foot">${esc(item.timing||'This week')} · ${esc(item.confidence||'unknown')} confidence</div></article>`).join('')}</div></section>`:'';
  const actionPlanHtml=model.actionPlan?.publishable&&Array.isArray(model.actionPlan.items)&&model.actionPlan.items.length?`<section class="section action-plan"><div class="eyebrow">What to do next</div><h2>90-day action plan</h2><p class="section-intro">${esc(model.actionPlan.intro)}</p><div class="action-plan-grid">${model.actionPlan.items.map((item,i)=>`<article class="action-step"><div class="action-step-top"><span class="action-rank">${esc(item.rank||i+1)}</span><div><div class="action-meta">Impact: ${esc(item.impact)} · Effort: ${esc(item.effort)} · Confidence: ${esc(item.confidence)}</div>${model.actionPlanPresentation?.items?.find(x=>x.id===item.id)?.readiness?`<div class="readiness-badge">${esc(model.actionPlanPresentation.items.find(x=>x.id===item.id).readiness.label)}</div>`:''}<h3>${esc(item.title)}</h3></div></div><p>${esc(item.action)}</p><div class="action-owner"><strong>Owner</strong><span>${esc(item.owner)}</span><strong>Suggested timing</strong><span>${esc(item.timing)}</span></div></article>`).join('')}</div>${model.evidenceAppendix?.href?`<a class="evidence-link" href="${esc(model.evidenceAppendix.href)}">View supporting evidence →</a>`:''}</section>`:'';
