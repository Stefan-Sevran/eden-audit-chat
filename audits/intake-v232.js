...
async function defaultModelTurn({apiKey,history,state,fetchImpl}){
  ...
  const response=await fetchImpl('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.EDEN_AUDIT_INTAKE_MODEL||'gpt-4.1-mini',temperature:.35,response_format:{type:'json_object'},messages:[{role:'system',content:humanConversationPrompt(state)},...history.slice(-24)]})});
...
function createAuditIntake({openaiApiKey='',fetchImpl=global.fetch,modelTurn=null,onConfirmed=async()=>({ok:true})}={}){
  ...
  async function handleText({sessionId,message}){const id=safeSessionId(sessionId);if(!id)throw new Error('A valid Audit session ID is required.');const user=text(message);if(!user)throw new Error('A message is required.');const state=ensure(id);const wasAwaitingConfirmation=state.awaitingConfirmation;const history=histories[id];history.push({role:'user',content:user});
    if(wasAwaitingConfirmation&&confirmationLike(user)&&!/@|\+?\d[\d\s()-]{7,}/.test(user)){state.ownerConfirmed=true;state.awaitingConfirmation=false;let confirmedSnapshot=buildSnapshot(state);let confirmedReply='All set. Where should I send the private Audit—email or WhatsApp?';if(confirmedSnapshot.contactAvailable){confirmedSnapshot=await maybeComplete(state);confirmedReply=`Your Audit intake is in. Reference: ${confirmedSnapshot.auditReference}. Eden will now scan the clinic, review the evidence, and prepare the private report.`;}history.push({role:'assistant',content:confirmedReply});histories[id]=history.slice(-50);return {reply:confirmedReply,snapshot:confirmedSnapshot};}
