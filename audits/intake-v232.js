...
function confirmationLike(value){
  const answer=text(value);
  if(!answer)return false;
  const alreadyConfirmed=/(?:already|ก่อนหน้านี้|na-confirm|nakumpirma|nakumpirmahan).{0,20}(?:confirm|ยืนยัน|confirmed)/i.test(answer)||/i (?:have|already) confirmed/i.test(answer);
  if(!alreadyConfirmed&&/(?:\b(?:but|except|change|adjust|wrong|incorrect|actually|hold on|not yet|no)\b|แต่|แก้|ไม่ถูก|hindi tama|pero|usba|sayop)/i.test(answer))return false;
  return alreadyConfirmed||/^(?:yes(?: please)?|yep|yeah|sure|absolutely|go ahead|proceed|send it|sounds good|looks good|all good|all correct|that'?s (?:right|fine|correct)|fair enough|fine by me|ok(?:ay)?|correct|confirmed|confirm|accurate|โอเค|ใช่|ถูกต้อง|ยืนยัน|ได้เลย|oo|opo|sige|tama(?: po)?|ayos|sakto|mao na|husto)(?:[,.! ]|$)/i.test(answer);
}
function yesLike(value){return confirmationLike(value);}
...
function confirmationText(snapshot){
  ...
  if(i.assumptions.length)lines.push('If Eden models anything beyond your figures, the report will label it clearly as an assumption—not a clinic fact.');
  return `Quick check before I run the Audit:\n\n${lines.join('\n')}\n\nDoes that look right?`;
}
...
function humanConversationPrompt(state){return `${socialInferencePrompt(state)}\n\nHuman voice rules: Sound like a capable, relaxed human colleague—not a form, call-centre script, or compliance bot. Use contractions and ordinary conversational vocabulary. Vary sentence openings. Do not begin every reply with “Great”, “Perfect”, “Thanks”, “Got it”, or “Noted”, and do not echo the person's number back on every turn. A short natural reaction is enough; sometimes move directly to the next useful question. Notice humour, hesitation, uncertainty, corrections, and side comments. Be precise about facts and calculations, but conversational everywhere else. Keep missed/delayed text messages separate from missed phone calls because they are separate Audit fields. If a value has two legitimate meanings—especially first-visit revenue versus total value across repeat visits—recognize the distinction and clarify which conservative figure should drive the Audit before presenting the confirmation summary.`;}
function deliveryWindow(){return text(process.env.AUDIT_DELIVERY_WINDOW)||'within one business day';}
function completionAwarePrompt(state){return `${humanConversationPrompt(state)}\n\nCompleted-intake rules: If the structured status is intake-complete, the intake is already safely recorded. Answer the owner’s new question directly and naturally. Do not repeat the receipt, ask for confirmation again, restart the interview, or claim that the finished report has already been sent. The normal delivery expectation is ${deliveryWindow()}, after a human evidence review.`;}
function completionReceipt(snapshot){return `Lovely—you’re in. Your reference is ${snapshot.auditReference}. We’ll scan the clinic, review the evidence, and send the private report ${deliveryWindow()}.`;}
async function defaultModelTurn({apiKey,history,state,fetchImpl}){
  if(!apiKey)throw new Error('OPENAI_API_KEY is not configured');
  const response=await fetchImpl('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.EDEN_AUDIT_INTAKE_MODEL||'gpt-4.1-mini',temperature:.35,response_format:{type:'json_object'},messages:[{role:'system',content:completionAwarePrompt(state)},...history.slice(-24)]})});
...
function realtimeHumanInstructions(snapshot){return `${realtimeInstructions(snapshot)}\n\nSpeak like a relaxed, capable human colleague. Use contractions, vary your openings, and stop saying “got it”, “noted”, “perfect”, or “thanks” after every answer. Don’t echo every number back unless checking ambiguity. React naturally to humour, uncertainty, corrections, and language switches. Keep missed text messages distinct from missed phone calls. Clarify first-visit value versus total repeat-visit value before final confirmation when the owner describes both.`;}
...
function needsPatientValueClarification(user,turn={}){
  const updates=cleanUpdates(turn.updates||turn);
  return !!updates.averageNewPatientValue&&/(?:first (?:visit|appointment)|initial visit|var(?:y|ies|iable)|rebook|repeat visit|multiple (?:times|visits)|long.?term|lifetime|total(?:ly)?|over time)/i.test(text(user));
}
function patientValueClarification(state,turn={}){
  const raw=text(cleanUpdates(turn.updates||turn).averageNewPatientValue)||text(state.answers.averageNewPatientValue);
  const amountMatch=raw.match(/\d[\d,.]*/);
  const amount=amountMatch?amountMatch[0]:raw;
  const currency=text(state.fields.currency)||inferCurrency(state.fields.clinicLocation,'');
  const symbol=currency==='THB'?'฿':currency==='PHP'?'₱':currency==='USD'?'$':'';
  const conservative=amount?`${symbol}${amount}${!symbol&&currency?` ${currency}`:''}`:'that first-visit amount';
  return `That distinction matters. For a conservative Audit, should I use ${conservative} as the first-visit value, or would you rather estimate the typical total value including repeat visits?`;
}
...
function createAuditIntake({openaiApiKey='',fetchImpl=global.fetch,modelTurn=null,onConfirmed=async()=>({ok:true})}={}){
  ...
  async function handleText({sessionId,message}){const id=safeSessionId(sessionId);if(!id)throw new Error('A valid Audit session ID is required.');const user=text(message);if(!user)throw new Error('A message is required.');const state=ensure(id);const wasAwaitingConfirmation=state.awaitingConfirmation;const history=histories[id];history.push({role:'user',content:user});
    if(state.delivered){const followUpTurn=await (modelTurn?modelTurn({history:clone(history),state:clone(state)}):defaultModelTurn({apiKey:openaiApiKey,history,state,fetchImpl}));const followUpReply=text(followUpTurn.reply)||`It normally arrives ${deliveryWindow()}. Your reference is ${state.auditReference}.`;history.push({role:'assistant',content:followUpReply});histories[id]=history.slice(-50);return {reply:followUpReply,snapshot:buildSnapshot(state)};}
    if(wasAwaitingConfirmation&&confirmationLike(user)&&!/@|\+?\d[\d\s()-]{7,}/.test(user)){state.ownerConfirmed=true;state.awaitingConfirmation=false;let confirmedSnapshot=buildSnapshot(state);let confirmedReply='That’s the numbers sorted. What’s the best email or WhatsApp for the private Audit?';if(confirmedSnapshot.contactAvailable){confirmedSnapshot=await maybeComplete(state);confirmedReply=completionReceipt(confirmedSnapshot);}history.push({role:'assistant',content:confirmedReply});histories[id]=history.slice(-50);return {reply:confirmedReply,snapshot:confirmedSnapshot};}
    const turn=await (modelTurn?modelTurn({history:clone(history),state:clone(state)}):defaultModelTurn({apiKey:openaiApiKey,history,state,fetchImpl}));applyUpdates(state,turn);const valueNeedsClarification=needsPatientValueClarification(user,turn);if(valueNeedsClarification)state.answered.averageNewPatientValue=false;const inferred=deriveObviousBasics(state,user);let snapshot=buildSnapshot(state);if(turn.ownerConfirmed===true&&snapshot.readyForConfirmation&&confirmationLike(user)){state.ownerConfirmed=true;state.awaitingConfirmation=false;}snapshot=buildSnapshot(state);
    let reply=text(turn.reply)||snapshot.nextQuestion||'Thank you. Let me confirm what I have.';
    if(valueNeedsClarification)reply=patientValueClarification(state,turn);
    ...
    if(snapshot.readyForConfirmation&&!state.ownerConfirmed){state.awaitingConfirmation=true;if(!wasAwaitingConfirmation||hasMaterialUpdates(turn))reply=confirmationText(snapshot);else if(questionLike(user))reply=text(turn.reply)||'Of course—what would you like me to clarify before you confirm?';}else if(state.ownerConfirmed&&!snapshot.contactAvailable){reply='That’s the numbers sorted. What’s the best email or WhatsApp for the private Audit?';}else if(state.ownerConfirmed&&snapshot.contactAvailable){snapshot=await maybeComplete(state);reply=completionReceipt(snapshot);}
    history.push({role:'assistant',content:reply});histories[id]=history.slice(-50);return {reply,snapshot};}
  async function handleVoice({sessionId,payload}){const id=safeSessionId(sessionId);if(!id)throw new Error('A valid Audit session ID is required.');const state=ensure(id);applyUpdates(state,{updates:payload,clearFields:payload.clearFields,answeredFields:payload.answeredFields});deriveObviousBasics(state,Object.values(payload||{}).filter(value=>typeof value==='string').join(' '));let snapshot=buildSnapshot(state);if(payload.ownerConfirmed===true&&snapshot.readyForConfirmation&&confirmationLike(payload.confirmationEvidence)){state.ownerConfirmed=true;state.awaitingConfirmation=false;}snapshot=buildSnapshot(state);if(state.ownerConfirmed&&snapshot.contactAvailable)snapshot=await maybeComplete(state);return {success:true,snapshot,summary:snapshot.readyForConfirmation?confirmationText(snapshot):null,instruction:snapshot.status==='intake-complete'?`Tell the owner naturally that the intake is safely received, give reference ${snapshot.auditReference}, and say the private report normally arrives ${deliveryWindow()} after human evidence review. Answer later follow-up questions without repeating this receipt.`:snapshot.ownerConfirmed?'Ask where to send the private Audit: email or WhatsApp.':snapshot.readyForConfirmation?'Read the concise summary, then ask the owner to confirm it.':snapshot.nextQuestion};}
  function install(app,expressLib){
    app.post('/audit-chat',async(req,res)=>{try{const result=await handleText(req.body||{});res.json({reply:result.reply,sessionId:result.snapshot.sessionId,intakeStatus:result.snapshot.status,nextField:result.snapshot.missingFields[0]||null,readyForConfirmation:result.snapshot.readyForConfirmation,confirmed:result.snapshot.ownerConfirmed,deliveryChannels:result.snapshot.delivery?.channels||null});}catch(error){const clientError=/required/i.test(String(error.message||error));if(!clientError)console.error('Audit intake chat error:',error);res.status(clientError?400:500).json({reply:clientError?'Please refresh the Audit page and try again.':'Small connection issue. Please try again in a moment.',error:clientError?'invalid_audit_request':'audit_intake_unavailable'});}});
