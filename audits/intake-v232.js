...
function asksKnownBasic(value,state){
  ...
  const patterns={
    ...
    websiteUrl:/(?:website|public page|url|valid link)/i
  };
...
function alignReplyWithState(reply,snapshot,state,inferred=[]){
  if(snapshot.status!=='collecting'||!snapshot.nextQuestion||!asksKnownBasic(reply,state))return text(reply);
...
function createAuditIntake({openaiApiKey='',fetchImpl=global.fetch,modelTurn=null,onConfirmed=async()=>({ok:true})}={}){
  ...
  async function handleText({sessionId,message}){const id=safeSessionId(sessionId);if(!id)throw new Error('A valid Audit session ID is required.');const user=text(message);if(!user)throw new Error('A message is required.');const state=ensure(id);const wasAwaitingConfirmation=state.awaitingConfirmation;const history=histories[id];history.push({role:'user',content:user});
    ...
    reply=alignReplyWithState(reply,snapshot,state,inferred);
...
module.exports={VERSION,ANSWER_IDS,UPDATE_IDS,safeSessionId,yesLike,applyUpdates,deriveObviousBasics,buildSnapshot,confirmationText,realtimeTool,realtimeInstructions,claimsPrematureCompletion,questionLike,hasMaterialUpdates,explicitPauseLike,replyHasQuestion,ensureForwardMotion,asksKnownBasic,alignReplyWithState,createAuditIntake};
